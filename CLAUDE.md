...
### Sync Architecture
- `backups/clipboard_sync.json` - Single file containing all current entries (gets overwritten)
- Archives created by `ArchiveManager` for entries older than 7 days
...
| `src/services/sync/github_sync.py` | `push_latest()`, `pull_latest()`, supports Enterprise via `enterprise_url` parameter |
