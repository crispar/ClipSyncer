...
class ClipboardHistoryApp:
    ...
    def _start_persistence_thread(self):
        """Start background thread for handling persistence tasks"""
        def persistence_worker():
            logger.info("Persistence worker thread started")
            while not self._shutdown_event.is_set():
                try:
                    # Get task from queue with timeout to allow checking shutdown event
                    try:
                        task = self._persistence_queue.get(timeout=1.0)
                    except queue.Empty:
                        continue
                    ...
                    if task_type == 'save_entry':
                        self._handle_save_entry(data)
                    elif task_type == 'archive_entry':
                        self._handle_archive_entry(data)
            ...
            logger.info("Persistence worker thread stopped")

        self._persistence_thread = threading.Thread(target=persistence_worker, daemon=True)
        self._persistence_thread.start()

    def _handle_save_entry(self, entry):
        """Handle saving entry to database (runs in background thread)"""
        try:
            ...
            logger.error(f"Failed to save entry in background: {e}")

    def _handle_archive_entry(self, entry_data):
        """Handle archiving entry (runs in background thread)"""
        try:
            if self.archive_manager:
                self.archive_manager.archive_entries([entry_data])
                logger.debug(f"Archived overflow entry: {entry_data.get('content_hash', '')[:8]}")
        except Exception as e:
    ...
    def _on_clipboard_change(self, content: str, timestamp):
        ...
        try:
            # Add to history (fast, in-memory)
            added, removed_entry = self.clipboard_history.add_entry(content, timestamp)

            # Archive the removed entry if any (queue for background processing)
            if removed_entry and self.archive_manager:
                try:
                    ...
                    self._persistence_queue.put(('archive_entry', archive_entry))
                except Exception as e:
                    logger.error(f"Failed to queue archive task: {e}")

            if added:
                # Queue database save
                entries = self.clipboard_history.get_entries(limit=1)
                if entries:
                    self._persistence_queue.put(('save_entry', entries[0]))

                # Show notification if enabled (fast signal emission)
                ...
                logger.debug(f"Clipboard content captured: {len(content)} characters")
