...
class GitHubSyncService(SyncBackend):
    ...
    def download_backup(self, filename: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # NOTE: ``data`` here is the encrypted blob ({ciphertext, nonce,
            # tag}); it does NOT have a top-level "entries" key. The real entry
            # count is logged by SyncCoordinator after decryption.
            logger.info(f"Downloaded backup: {filepath} ({len(content)} bytes, encrypted)")
            return data
